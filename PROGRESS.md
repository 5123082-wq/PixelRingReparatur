...
- Date: 2026-09-11
- Current stage: Локально реализованы три сценария связи публичной заявки с кабинетом: автоматическое добавление при действующем входе, необязательное создание кабинета после гостевой заявки и явное подтверждение добавления для существующего аккаунта через почту. Гостевой ответ одинаков, письма различаются, существующий пароль сохраняется. Сквозные сценарии, письма на шести языках, проверка типов, качества кода и промышленная сборка пройдены; подробности в [журнале кабинета](docs/04_client_portal/client_portal_implementation_plan.md#progress-log).
- Next action: Владелец отдельно выбирает выпуск изменений. [Варианты защиты от массовой отправки](docs/04_client_portal/request_intake_abuse_protection_options_2026-09-11.md) исследованы дополнительным агентом; защита пока не изменена.

- Date: 2026-09-05
- Current stage: Первая волна исправлений безопасности S-01/S-02 локально реализована и прошла E2E-проверку (сквозную проверку) через реальные маршруты на одноразовой PGlite (встроенной PostgreSQL-совместимой БД) после применения всех 27 существующих миграций. Знание номера и контакта теперь даёт только безопасный статус без новой сессии; ссылка активации и общий чат требуют сильного доступа, токены кабинета и ограниченного статуса не повышают права, история чата ограничена одной явно разрешённой заявкой, а отключённый пользователь и отозванное право кабинета не обходятся через другой cookie (файл сессии в браузере). Новые и существующие тесты, проверка типов TypeScript, ESLint (проверка качества кода), независимые поиски обхода и промышленная сборка пройдены; миграций схемы и изменений зависимостей нет.
- Next action: Владелец отдельно выбирает подготовку коммита и плана развёртывания S-01/S-02 либо запуск блока 2 для S-03/S-04. До выпуска нужно отдельно согласовать судьбу ещё действующих ссылок активации, которые мог раскрыть прежний маршрут, и выполнить проверку после развёртывания. Текущий код не закоммичен и не развёрнут.

- Date: 2026-09-05
- Current stage: Завершена проверка безопасности и подготовлен [единый документ передачи с планом исправлений](docs/10_security_privacy/security_audit_and_remediation_handoff_2026-09-05.md): 9 подтверждённых проблем приложения и отдельно 21 уязвимая зависимость по данным npm. Исправления кода не начаты.
- Next action: Согласовать первый блок исправления границ доступа к статусу, переписке и активации кабинета; продолжать по журналу единого документа.
