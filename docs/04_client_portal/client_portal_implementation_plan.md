...
- 2026-09-11: public website intake locally supports authenticated automatic linking, optional account creation after guest intake, and explicit email-confirmed linking for an existing account without changing its password. See the latest Progress Log; production rollout is separate.
...
- **Текущий блок:** связь заявок с кабинетом при отправке через публичный сайт.
- **Сделано:** заявка регистрируется сразу. При действующей сессии кабинета привязывается к пользователю этой сессии; введённый контакт не выбирает владельца кабинета. Сохраняется отдельная сессия заявки/чата, перенос сообщений и вложений ограничен текущим черновиком. Без входа наличие аккаунта влияет только на письмо: создание кабинета или добавление заявки; публичный ответ и сообщение едины. Письма и сообщения подготовлены на DE/EN/RU/TR/PL/AR (немецком, английском, русском, турецком, польском и арабском), для арабского сохранено направление справа налево.
- **Подтверждение:** открытие ссылки и проверка кода сами по себе не добавляют заявку. Новый пользователь явно создаёт кабинет, существующий явно подтверждает добавление без смены пароля. Повторное использование завершённого подтверждения отклоняется. Проверка происхождения запроса добавлена к публичной отправке и изменяющим доступ действиям подключения.
- **Уточнения по проверке изменений №76:** результат первой и повторной заявки из чата сохраняется в истории вместе с признаком добавления в кабинет; карточка после обновления истории показывает номер и правильное действие. Приглашения из Telegram без явно выбранного сценария сохраняют нейтральный текст. Добавлены проверки повторной загрузки истории для вошедшего и гостевого пользователя, а также писем из Telegram для существующего и нового аккаунта.
- **Проверено:** три сквозных теста через реальные HTTP-маршруты (серверные адреса приложения) и локальный SMTP-перехватчик (тестовый почтовый сервер) на одноразовой PGlite (встроенной PostgreSQL-совместимой базе) после применения существующих миграций. Проверены три сценария, одинаковая структура гостевых ответов, различие писем, отсутствие привязки до подтверждения, сохранение пароля, повтор подтверждения, недействительные/отозванные/просроченные сессии, отключённый аккаунт, контакт другого аккаунта, новая заявка при старой сессии заявки, вложения чата и шесть языков писем. Существующие проверки границ доступа также пройдены. Малые тесты кабинета и публичного приёма: 39 из 39; проверка типов TypeScript, ESLint (проверка качества кода) и промышленная сборка пройдены.
- **В работе:** локальная реализация завершена; выпуск отдельно не согласован.
- **Следующее действие:** владелец выбирает выпуск изменений. Защита от массовых заявок изучена дополнительным агентом; варианты сохранены в [отдельном исследовании](request_intake_abuse_protection_options_2026-09-11.md), реализация не начата.
- **Блокеры/риски:** рабочая база не изменялась; ранее созданные непривязанные заявки автоматически не переносились. Доставка реальных писем и проверка после развёртывания остаются отдельными действиями. Ошибка отправки письма не отменяет уже зарегистрированную заявку; надёжная очередь повторной доставки пока не реализована. Защита от массовой отправки остаётся отдельным блоком.
- **Обновлённые документы:** этот план, README.md (указатель раздела), исследование защиты от массовых отправок, PROGRESS.md (глобальный журнал).
