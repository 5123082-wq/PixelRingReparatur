...
### 2026-09-08 — Единый вид всех семи карточек услуг

- Дата и текущий блок: 2026-09-08, перенос утверждённого оформления карточки ремонта на остальные услуги с подтверждённым сохранением шахматного порядка.
- Сделано: общий заголовок и подпись категории над двумя колонками, единые отступы и медный акцент у описания, стрелки у ссылок, совмещение нижнего края кнопки и фотографии. Пропорции колонок зеркально меняются вместе с расположением фото; на телефоне фото всегда перед описанием. Недостающие подписи категорий добавлены для всех шести языков.
- В процессе: визуальная оценка владельцем всей страницы.
- Следующее действие: проверить результат на локальной странице услуг.
- Проверки и риски: в немецкой версии проверены все семь карточек — фото чередуются справа/слева, отклонение верхних и нижних линий составляет 0 px. При ширине 390 px все фото перед описанием, кнопки на ширину колонки, горизонтального переполнения нет. Целевая проверка ESLint и проверка формата изменений пройдены. Другие языки визуально не проверялись.
- Обновлённые документы: этот журнал и код страницы `signage-service/src/app/[locale]/leistungen/page.tsx`. Правка вёрстки не добавлена в краткий глобальный журнал.

### 2026-09-08 — Пробное выравнивание карточки ремонта

- Дата и текущий блок: 2026-09-08, локальная доработка одной карточки на странице услуг по подтверждению владельца.
- Сделано: только для `repair-maintenance` верх описания совмещён с верхом фото, кнопка выровнена по началу текста и нижнему краю фото; добавлена декоративная стрелка с разворотом для арабского языка. На телефоне фото расположено перед описанием, кнопка занимает ширину колонки. Остальные карточки сохранены.
- В процессе: визуальная оценка владельцем.
- Следующее действие: получить оценку одной карточки; перенос на остальные услуги требует отдельного подтверждения.
- Проверки и риски: немецкая версия проверена в браузере на компьютере и при ширине 390 px, горизонтального переполнения нет; целевая проверка ESLint и проверка формата изменений пройдены. Другие языки визуально не проверялись.
- Обновлённые документы: этот журнал; код страницы `signage-service/src/app/[locale]/leistungen/page.tsx`. Краткий глобальный журнал не дополнен, поскольку это небольшая правка вёрстки.
