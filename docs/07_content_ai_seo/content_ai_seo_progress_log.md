...
* **Date:** 2026-08-21
* **Current sprint/block:** Утверждённое владельцем исправление технических причин слабой индексации после независимого аудита Google Search Console (панели Google для владельцев сайта).
* **Done:**
  - Шесть изолированных языковых компонентов ссылочного графа заменены одним связным графом из 132 URL. Закрытый переключатель языков теперь выводит настоящие ссылки DE/EN/RU/TR/PL/AR уже в исходной серверной разметке, отключает `prefetch` (предварительную загрузку) для скрытых ссылок, сохраняет сопоставление локализованных маршрутов, поддерживает Arabic RTL (арабское направление справа налево) и предоставляет доступную раскрываемую навигацию. Страницы статей показывают только реально опубликованные языковые варианты.
  - На всех 42 опубликованных статьях добавлены серверные контекстные внутренние ссылки на локализованный обзор проблем, подходящую страницу услуги и `Referenzen` (страницу примеров работ). Двенадцать намеренно оставленных маршрутов `noindex, follow` (запрет индексации при разрешённом переходе по ссылкам) теперь направляют навигацию статей сразу на опубликованный английский контент и создают ноль ссылок между неопубликованными вариантами.
  - Статическая генерация с заданным сроком и ISR (инкрементальная статическая регенерация) расширены с 30 до 114 из 132 публичных URL карты сайта: шесть главных страниц обновляются за пять минут, ещё 108 публичных страниц — за один час. Оставшиеся 18 динамических страниц — шесть маршрутов `Referenzen` (страницы примеров работ) и двенадцать юридических маршрутов; закрытые маршруты кабинета и административных панелей остаются динамическими и используют `private, no-store` (закрытое кеширование без сохранения).
  - `sitemap.xml` (карта сайта) обновляется каждые пять минут, сохраняет последнюю успешную версию при ошибках запроса к базе и останавливает промышленную сборку при отсутствии обязательной конфигурации базы вместо молчаливого исключения всех 42 статей CMS (системы управления контентом). Кешированное получение статьи теперь даёт метаданным и основному тексту один согласованный снимок публикации.
  - Добавлены прямые постоянные перенаправления для `/`, `/leistungen` и старых административных адресов с сохранением параметров запроса. Служебные проверки Apple теперь получают прямой пустой ответ 404 без языковых перенаправлений и заголовков альтернатив. Добавлен корректный `/favicon.ico` (значок сайта), а девять ссылок с публичных TR/PL/AR‑страниц услуг и три ссылки с главных страниц перенаправлены с неопубликованной статьи на локализованный обзор проблем. Заменено отсутствовавшее изображение Open Graph (изображение для соцсетей), а объявленные размеры синхронизированы с фактическими 1672×941.
  - Итоговое состояние проверено 23 точечными тестами Node, ESLint (проверкой качества кода; 0 ошибок и 26 существовавших предупреждений), TypeScript (проверкой типов), `git diff --check` (проверкой формата изменений) и успешной промышленной сборкой Next.js 16.2.6, которая создала 278 маршрутов и 42 опубликованные страницы статей. Полный локальный обход подтвердил: карта сайта содержит 132 URL; 132/132 возвращают HTTP 200 (успешный ответ); 132/132 имеют каноническую ссылку на саму страницу; среди них нет запрета индексации; граф состоит из одной связной компоненты из 132 URL и содержит 624 межъязыковые ссылки; 42/42 статей имеют полный блок контекстных ссылок; все 132 страницы создают ноль ссылок на статьи вне карты сайта; резервный кластер также создаёт ноль внутренних ссылок.
  - Интерактивная проверка в браузере подтвердила содержательную отрисованную страницу, отсутствие окна ошибки фреймворка, шесть доступных для робота языковых ссылок, правильную локализованную доступную подпись, открытие и закрытие, возврат фокуса по клавише Escape, фильтрацию по опубликованным языкам и отсутствие ссылок на резервные статьи. Единственные локальные ответы 404 в консоли пришли от скрипта Vercel Insights (аналитики Vercel), который обслуживается на Vercel и ожидаемо отсутствует в локальном `next start` (промышленном сервере Next).
* **In progress:** В этом утверждённом блоке не осталось работ внутри репозитория. Фиксация изменений, развёртывание и действия в Google Search Console (панели Google для владельцев сайта) намеренно не выполнялись без отдельного разрешения.
* **Next action:** После отдельного подтверждения владельца зафиксировать и развернуть проверенные файлы; проверить на рабочем сайте ответы `x-vercel-cache` (заголовок кеша Vercel), перенаправления, прямые ответы 404, значок сайта, все 132 маршрута карты сайта и связный языковой граф. Повторно отправить `https://www.pixel-ring.com/sitemap.xml` (карту сайта), выгрузить примеры URL для каждой причины исключения и сравнить `Discovered, currently not indexed` (обнаружено, но пока не проиндексировано) по языкам через 7, 14 и 28 дней.
* **Blockers/risks:** Изменения устраняют подтверждённые дефекты обнаружения и эффективности обхода, но не гарантируют индексацию и не доказывают одну-единственную причину исторических исключений. Google Search Console (панель Google для владельцев сайта) ранее показывала 143 обработанных URL при текущих 132 URL карты сайта, поэтому исторические URL и URL вне карты требуют выгрузки соответствия между причиной и адресом. Временное перенаправление домена без `www` на `www` остаётся на стороне Vercel. Десять юридических страниц не на немецком всё ещё повторяют немецкий основной текст, а шесть страниц `Referenzen` (примеров работ) формируются при каждом запросе; перевод, доверие, источники и внешняя авторитетность остаются отдельными направлениями качества контента, но не блокируют этот выпуск кода.
* **Updated documents and code:** Root `PROGRESS.md` (краткий глобальный журнал), domain `README.md` (доменный файл-ориентир), this domain progress log (этот доменный журнал), localized layout/home/article routing (локализованные макет, главная и страницы статей), language switcher/header (переключатель языков и шапка), article context-link model/body (модель и отображение контекстных ссылок статей), sitemap and proxy/redirect configuration (карта сайта и конфигурация прокси/перенаправлений), homepage case links (ссылки блока примеров на главной), favicon and focused regression tests (значок сайта и точечные регрессионные тесты).
...
* **Done:**
  ...
  - Removed `cookies()` (серверное чтение cookie-файла), Prisma (слой доступа к базе данных) and portal authentication (проверку авторизации кабинета) from the public homepage render. Added `generateStaticParams` (предварительное создание языковых маршрутов), `setRequestLocale` (фиксацию языка запроса), five-minute revalidation (обновление раз в пять минут) and disabled automatic locale cookies (автоматические языковые cookie-файлы). The production manifest (манифест промышленной сборки) confirms `initialRevalidateSeconds: 300` (обновление через 300 секунд) for `/de`, `/en`, `/ru`, `/tr`, `/pl` and `/ar`.
  - Preserved the authenticated header behavior through a separate private `GET /api/portal/session-state` (служебный запрос состояния входа). It returns only `{authenticated: boolean}` (логический признак «выполнен вход»), uses `private, no-store` (запрет общего и браузерного кеширования), varies by cookie (разделяет ответы по cookie-файлу), and is excluded from indexing. The client header checks it after hydration (после загрузки страницы), keeps the neutral status link before/without authentication, and changes it to the localized portal link only on exact `true` (истинном значении).
  - Kept ordinary portal access checks (обычные проверки доступа в кабинет) authoritative and session-touching (обновляющими время активности), while the header-only presentation probe (проверка только для отображения шапки) validates the session without changing `lastSeenAt` (время последней активности).
