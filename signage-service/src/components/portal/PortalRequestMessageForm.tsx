...
export default function PortalRequestMessageForm({
  ...
  async function sendMessage(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setBody('');
      setFeedback('Nachricht wurde gespeichert.');
      router.refresh();
    } catch (error) {
  ...
  return (
    ...
    >
      <label className="block">
        <span className={isChat ? 'sr-only' : 'text-[13px] font-black text-[#B8643E]'}>Nachricht an PixelRing</span>
        <textarea
          value={body}
      ...
      <div className={isChat ? 'flex flex-wrap items-center justify-between gap-3' : 'mt-3 flex flex-wrap items-center gap-3'}>
        <span className="text-[12px] font-semibold text-[#8A7467]">Dateianhaenge folgen in einem separaten sicheren Schritt.</span>
        <button
          type="submit"
          disabled={isSubmitting}
