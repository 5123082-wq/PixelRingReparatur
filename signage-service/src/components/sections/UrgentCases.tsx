          ...
          <div className={`relative z-10 grid ${gridLayout} gap-8 items-center`}>
            <div>
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-md rounded-full mb-6 border border-white/20">
                ...
                <span className="text-[12px] md:text-[14px] font-bold tracking-wider uppercase">
                  {content?.badge ?? t('urgent_badge')}
                </span>
              </div>
              
              <h2 className={`${titleSize} font-bold mb-4 leading-tight`}>
                {content?.title ?? t('urgent_cases_title')}
              </h2>
              
              <p className={`${descSize} text-white/90 leading-relaxed ${isSidebar ? 'mb-6' : 'mb-10'}`}>
                {content?.intro ?? t('urgent_cases_desc')}
              </p>
            </div>
            ...
            <div className={`flex ${buttonLayout} gap-4`}>
              ...
              >
                ...
                {content?.primaryLabel ?? t('call_technician')}
              </button>
              ...
              >
                {content?.secondaryLabel ?? t('send_request')}
              </button>
            </div>
          </div>
        </div>
